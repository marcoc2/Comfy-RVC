...
rvc_model_dir = os.path.join(BASE_MODELS_DIR, "RVC")
os.makedirs(rvc_model_dir, exist_ok=True)
os.makedirs(os.path.join(rvc_model_dir, ".index"), exist_ok=True)
folder_paths.add_model_folder_path("rvc_models", rvc_model_dir)
folder_paths.add_model_folder_path("rvc_index", os.path.join(rvc_model_dir, ".index"))

class LoadRVCModelNode:

    @classmethod
    def INPUT_TYPES(cls):
        local_pths = [f"RVC/{f}" for f in folder_paths.get_filename_list("rvc_models") if f.endswith(".pth")]
        model_list = list(set(RVC_MODELS + local_pths))
        local_index = [f"RVC/.index/{f}" for f in folder_paths.get_filename_list("rvc_index") if f.endswith(".index")]
        index_list = list(set(["None"] + RVC_INDEX + local_index))

        return {
            'required': {
                'model': (sorted(model_list),{"default": model_list[0] if model_list else ""}),
            },
            "optional": {
                "index": (sorted(index_list),{"default": "None"}),
            }
        }
